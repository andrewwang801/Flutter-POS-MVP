...
import 'i_print_repository.dart';

@Injectable(as: IPrintRepository)
class PrintLocalRepository extends IPrintRepository
    with TypeUtil, StringUtil, DateTimeUtil {
  PrintLocalRepository({required this.dbHelper});
  ...
  Future<String> generateMasterKP(int masterKPID, String ctgName, int ctgID,
      String tblNo, int salesNo, int splitNo, int masterID) async {
    final Database dbHandler = await dbHelper.database;
    String strMasterKP = '', query = '';
    final int crntPrLen = 20;
    final String strKPId = 'KP1';

    if (POSDtls.KPOrderRemarks) {
      query =
          'SELECT IFNULL(MAX(KPOrderNo),0) FROM KPStatus WHERE SalesNo = $salesNo AND SplitNo = $splitNo';
      List<Map<String, dynamic>> data = await dbHandler.rawQuery(query);
      if (data.isEmpty) {
        return '';
      }
      ...
      if (kpOrderNo > 0) {
        ...
        if (data.isNotEmpty) {
          ...
          strMasterKP +=
              '${textPrintFormat('N', 'L', '2')}${addSpace(remarks, space)}\n';
        }
      }
    }
    query =
        'SELECT AutoCheck FROM Operator WHERE OperatorNo = ${GlobalConfig.operatorNo}';
    List<Map<String, dynamic>> array = await dbHandler.rawQuery(query);
    strMasterKP += textPrintFormat('N', 'L', '2') + '\n';
    strMasterKP += textPrintFormat('n', 'L', '2') + ctgName + '\n';
    if (POSDtls.blnKPMasterPrintID) {
      strMasterKP += textPrintFormat('N', 'L', '2') + '\n';
      strMasterKP +=
          textPrintFormat('N', 'L', '2') + 'KPID : ' + strKPId + '\n';
    }
    ...
    if (array.isNotEmpty) {
      ...
      if (blnAutoCheck) {
        ...
        strMasterKP +=
            textPrintFormat('N', 'L', '2') + POSDtls.LabelTable + tblNo + '\n';
      }
    }

    if (POSDtls.MasterKPPrintTblRemarks) {
      query =
    ...
    strMasterKP += textPrintFormat('N', 'L', '2') + addDash(crntPrLen) + '\n';

    String masterPrintStr = 'MasterKPPrint';
    if (masterID != 1) {
    ...
    for (int i = 0; i < itemArray.length; i++) {
      ...
      if (funcID == 101 && transStatus == 'V') {
        ...
        if (transStatus == 'V' ||
            ...
            prep ||
            comments) {
          if (transStatus == 'M' || !POSDtls.blnKPPrintPrepQty || comments) {
            iName = addSpace(iName, 4);
            strMasterKP += '${textPrintFormat('N', 'L', '2')}$iName \n';
          } else {
            strMasterKP += '${textPrintFormat('N', 'L', '2')}$strQty $iName \n';
          }

          if (iName2.isNotEmpty) {
            strMasterKP += '${textPrintFormat('N', 'L', '2')}$iName2 \n';
          }
        } else {
          if (qty == 0) {
            iName = addSpace(iName, 4);
        ...
        query =
            'SELECT instruction FROM HeldItems WHERE SalesNo = $salesNo AND SplitNo = $splitNo AND TransStatus = "V" AND SalesRef = $salesRef';
        final List<Map<String, dynamic>> orderKpArray =
            await dbHandler.rawQuery(query);
        if (orderKpArray.isNotEmpty) {
          if (POSDtls.PrintVoidRemarksKP) {
            ...
            strMasterKP += '${textPrintFormat('N', 'L', '2')}\n';
            strMasterKP += '${textPrintFormat('N', 'L', '2')}$instruction\n';
          }
        }
      }
    }
    ...
    strMasterKP += currentDateTime('dd/MM/yyyy HH:mm');
    ...
    if (GlobalConfig.TransMode == 'RFND') {
      strMasterKP += '${textPrintFormat('N', 'L', '2')}\n';
      strMasterKP += '${textPrintFormat('N', 'L', '2')}******REFUND******\n';
    }
    strMasterKP += '${textPrintFormat('N', 'L', '2')}\n';
    strMasterKP += '${textPrintFormat('N', 'L', '2')}\n';
    return strMasterKP;
